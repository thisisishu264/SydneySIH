...
// --- CHANGE #1: Updated the Scheme interface to match your database columns ---
interface Scheme {
    scheme_id: number;
    scheme_name: string;
    description: string;       // Was 'scheme_description'
    benefit_highlight: string; // Was 'benefit_summary'
}
...
const HomePage = () => {
    ...
    useEffect(() => {
        // In a real app, you would get the full user object on login
        const storedUserName = localStorage.getItem('user_name') || "Test User";
        const storedUserId = localStorage.getItem('user_id') || "3"; // Using user 3 for testing
        ...
        const fetchSchemes = async () => {
            try {
                // --- CHANGE #2: Updated the fetch URL to match your backend API route ---
                const response = await fetch(`${API_BASE_URL}/schemes/eligible/${storedUserId}`);
    ...
    const handleApply = async (schemeId: number, schemeName: string) => {
        const userId = localStorage.getItem('user_id') || "3"; // Using user 3 for testing

        try {
            // --- CHANGE #3: Updated the fetch URL and body to match your backend ---
            const response = await fetch(`${API_BASE_URL}/applications`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId: parseInt(userId), schemeId: schemeId }),
            });
                    ...
                    <Card key={scheme.scheme_id} className="shadow-md transition-all hover:shadow-lg">
                        <CardHeader>
                            ...
                            <CardDescription className="text-lg font-semibold text-green-700">
                                {/* --- CHANGE #4: Updated property name --- */}
                                {scheme.benefit_highlight}
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            {/* --- CHANGE #5: Updated property name --- */}
    ...
    // ... The rest of your JSX for rendering the page remains the same ...
    return (
